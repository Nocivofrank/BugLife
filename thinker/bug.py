import pygame, chunks as ch, consumable.food as food
...
class Bug():
    ...
    def __init__(self, id , pos = pygame.Vector2(100 , 100), energy_passed = 1000, brain = None):
        ...
        self.brain_think_cooldown = 1
        self.brain_think_cooldown_timer = Brain.Brain.random_range(0, 1)
    ...
    def update_detect(dt, game_chunk):
        for bug in Bug.bugs:
            ...
            chunk = game_chunk.chunk_lists["bug_chunks"]

            if bug.quadrant is not None:
                list = chunk[bug.quadrant]
                if list is not None:
                    for i in range(len(list)):
                        ...
                        if distance < Bug.detection_range:
                            ...
                            if None in bug.closest_other:
                                ...
                                farthest_index = max(range(Bug.amount_detect_max),key=lambda i: bug.pos.distance_to(bug.closest_other[i].pos))

                                if distance < bug.pos.distance_to(bug.closest_other[farthest_index].pos):
            ...
            chunk = game_chunk.chunk_lists["fruit_chunks"]
    ...
    def update(dt, screen, wrap = True):
        for bug in Bug.bugs:
            if bug.energy <= 0:
                bug.alive = False
                continue
    ...
    def think(dt):
        for bug in Bug.bugs:
            if bug.energy <= 0:
                ...
                if bug.brain_think_cooldown_timer >= bug.brain_think_cooldown:
                    out, energy_consumed = bug.brain.brainThink()
                    bug.energy -= energy_consumed
                    bug.brain_think_cooldown_timer = 0
            ...
            if draw_debug:
                for i in range(Bug.amount_detect_max):
                    if bug.closest_other[i] != None:
                        ...
                        if distance <= Bug.detection_range:
                            pygame.draw.aaline(screen, (0 , 255, 0 , 100), bug.pos, bug.closest_other[i].pos)
